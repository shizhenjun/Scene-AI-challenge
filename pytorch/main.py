...
def submit(**kwargs):
    ...
    for ii, data in tqdm.tqdm(enumerate(dataloader)):
        ...
        softlogits, score = model(val_input)
...
def train(**kwargs):
    ...
    for epoch in range(opt.max_epoch):
        ...
        for ii, data in tqdm.tqdm(enumerate(dataloader, 0)):
            ...
            softlogits, score = model(input)
            error = criterion(score, label)
            error.backward()
            optimizer.step()

            # 计算损失的均值和训练集的准确率均值
            loss_meter.add(error.data[0])
            acc = topk_acc(score.data,label.data)
            acc_meter.add(acc)
            top1_acc = topk_acc(score.data,label.data,k=1)
